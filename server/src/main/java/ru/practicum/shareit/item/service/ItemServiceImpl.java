...
public class ItemServiceImpl implements ItemService {
    ...
    public ItemResponseDto createItem(Long userId, ItemRequestDto itemDto) {
        ...
        if (itemDto.getRequestId() != null) {
            log.info("Создание вещи в ответ на запрос id: {}", itemDto.getRequestId());

            ItemRequest request = itemRequestRepository.findById(itemDto.getRequestId())
                    .orElseThrow(() -> new NotFoundException("Запрос с id " + itemDto.getRequestId() + " не найден"));

            // Проверяем, что пользователь НЕ является автором запроса
            if (request.getRequester().getId().equals(userId)) {
                log.warn("Пользователь {} попытался ответить на свой же запрос {}", userId, itemDto.getRequestId());
                throw new BadRequestException("Нельзя ответить на свой собственный запрос");
            }

            item = ItemMapper.toItemWithRequest(itemDto, user, request);
        } else {
            // Создание вещи без привязки к запросу
            item = ItemMapper.toItem(itemDto, user);
        }

        Item savedItem = itemRepository.save(item);
        log.info("Вещь успешно создана с id: {}, владелец: {}", savedItem.getId(), userId);
